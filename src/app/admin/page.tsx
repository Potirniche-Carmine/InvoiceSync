  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!validationResponse.ok) {
        window.location.href = `/admin?error=${encodeURIComponent("Security check failed. Please try again.")}`;
        return;
      }
      ...
      if (result?.error) {
        window.location.href = `/admin?error=${encodeURIComponent(result.error)}`;
        return; 
      } else {
        window.location.href = "/admin/dashboard";
      }
    } catch {
      window.location.href = `/admin?error=${encodeURIComponent("An error occurred. Please try again.")}`;
      return; 
    }
